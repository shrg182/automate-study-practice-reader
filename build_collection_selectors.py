...
def source_catalog(root: Path) -> list[dict[str, str]]:
    path = root / "source_catalog.csv"
    if not path.exists():
        return []
...
def merged_rows(root: Path) -> list[dict[str, str]]:
    editors = [editor_data(path, root) for path in sorted(root.glob("*/editor.html"))]
    editor_by_url = {row["source_url"].rstrip("/"): row for row in editors if row["source_url"]}
    editor_by_id = {row["id"]: row for row in editors}
    rows: list[dict[str, str]] = []
    used: set[str] = set()
    for source in source_catalog(root):
        editor = editor_by_url.get(source["source_url"].rstrip("/")) or editor_by_id.get(source["id"])
        rows.append({
            "id": source["id"],
            "title": source["title"],
            "source_url": source["source_url"],
            "section": source.get("section", ""),
            "editor": editor["editor"] if editor else "",
        })
        if editor:
            used.add(editor["id"])
    rows.extend({**row, "section": "其他已生成篇目"} for row in editors if row["id"] not in used)
    return rows
...
def build(key: str, title: str, unit: str, description: str) -> None:
    ...
    print(f"Wrote {output} with {len(rows)} selectable entries")
