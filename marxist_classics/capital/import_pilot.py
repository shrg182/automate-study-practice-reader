...
from build_editor import build_html, load_inline_notes, load_reading_notes, load_review_notes, load_terms, localize_english_first_ui  # noqa: E402
...
.bilingual-layout-control{position:relative;display:inline-flex}.bilingual-layout-trigger{white-space:nowrap}.bilingual-layout-popover{position:absolute;z-index:360;top:calc(100% + 6px);left:0;display:none;width:min(440px,calc(100vw - 24px));padding:12px;border:1px solid #c9d2df;border-radius:10px;background:#fff;box-shadow:0 10px 30px #0003}.bilingual-layout-control.open .bilingual-layout-popover{display:grid;gap:11px}.bilingual-presets{display:grid;grid-template-columns:repeat(5,1fr);gap:4px}.bilingual-presets button{min-width:0!important;padding:6px 3px!important;font-size:11px!important;line-height:1.2!important;white-space:normal}.bilingual-presets button.active{background:#e6f4ea!important;color:#137333!important;font-weight:700}.bilingual-slider-label{display:grid;grid-template-columns:auto minmax(140px,1fr) auto;gap:8px;align-items:center;color:var(--muted);font:12px/1.3 system-ui,sans-serif}.bilingual-slider-label>span{white-space:nowrap}.bilingual-slider-label input{width:100%;min-width:80px}.bilingual-layout-output{grid-column:1/-1;color:var(--ink);font-weight:700;white-space:nowrap}
...
'''
    return localize_english_first_ui(output.replace("</style>", css + "</style>", 1).replace("</body>", script + "</body>", 1))
