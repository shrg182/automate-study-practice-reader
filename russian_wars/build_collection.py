...
def editing_toolbar() -> str:
    return '''<nav class="toolbar" aria-label="Инструменты редактирования"><button type="button" data-command="undo">Отменить</button><button type="button" data-command="redo">Повторить</button><button type="button" data-command="bold"><b>Ж</b></button><button type="button" data-command="italic"><i>К</i></button><button type="button" data-command="highlight">Выделить</button><button type="button" data-command="removeFormat">Очистить формат</button><button type="button" data-command="save">Сохранить текст</button></nav>'''
...
def main() -> None:
    for item in ARTICLES:
        ...
        (folder / "editor.html").write_text(reader_page(item), encoding="utf-8")
        for index, battle in enumerate(BATTLES.get(f"{item['id']}@{item['period']}", []), 1):
            ...
            (battle_folder / "editor.html").write_text(battle_page(item, battle, index), encoding="utf-8")
