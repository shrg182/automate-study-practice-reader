...
def editing_toolbar() -> str:
    return '''<nav class="toolbar" aria-label="Инструменты редактирования"><div class="toolbar-modes" role="group" aria-label="Набор инструментов"><button type="button" data-toolbar-mode="full">Полная</button><button type="button" data-toolbar-mode="simple">Упрощённая</button></div><div class="toolbar-group"><button type="button" data-command="undo">Отменить</button><button type="button" data-command="redo">Повторить</button><button type="button" data-command="bold"><b>Ж</b></button><button type="button" data-command="italic"><i>К</i></button><button type="button" data-command="underline"><u>Ч</u></button><button type="button" data-command="highlight">Выделить</button><button type="button" data-command="removeFormat">Очистить</button><button type="button" data-command="save">Сохранить</button></div><div class="toolbar-group full-only"><button type="button" id="wordAnnotation">Аннотация слова</button><button type="button" id="speakText" class="primary">Читать вслух</button><button type="button" id="pauseSpeech" disabled>Пауза</button><button type="button" id="stopSpeech" disabled>Стоп</button><select id="speechVoice" aria-label="Голос"><option value="">Системный голос</option></select><label class="speech-rate">Скорость <input id="speechRate" type="range" min="0.5" max="1.5" step="0.1" value="0.8"><output id="speechRateValue">0.8×</output></label></div></nav>'''
...
function loadVoices(){const current=voiceSelect.value,voices=speechSynthesis.getVoices();voiceSelect.innerHTML='<option value="">Системный голос</option>'+voices.map((voice,index)=>`<option value="${index}">${voice.name} · ${voice.lang}</option>`).join('');if([...voiceSelect.options].some(option=>option.value===current))voiceSelect.value=current}
