class JianshangEditorHeader extends HTMLElement {
  connectedCallback() {
    ...
    if (!document.querySelector('script[src$="workspace_skin.js"]')) {
      ...
      skin.src = "../../workspace_skin.js?v=103";
    ...
    this.innerHTML = `
      <header>
        <div class="topbar">
          ...
          <div class="actions">
            <div class="header-tool-group view-tools" aria-label="导航与视图"><a class="action-link" href="../../index.html">返回首页</a><button id="viewToggleBtn" class="primary" type="button">切换为注音稿</button><span id="modeBadge" class="mode-badge">当前：清稿</span><div class="source-layout-control"><button id="sourceLayoutButton" type="button" aria-haspopup="menu" aria-expanded="false">分栏布局</button><div class="source-layout-popover" id="sourceLayoutPopover" role="menu" aria-label="正文与 PDF 布局"><button type="button" role="menuitem" data-source-layout="text">正文</button><button type="button" role="menuitem" data-source-layout="text-primary">正文为主</button><button type="button" role="menuitem" data-source-layout="balanced">均衡</button><button type="button" role="menuitem" data-source-layout="pdf-primary">PDF为主</button><button type="button" role="menuitem" data-source-layout="pdf">PDF</button></div></div></div>
