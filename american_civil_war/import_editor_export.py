#!/usr/bin/env python3
"""Import an American Civil War year-reader backup and rebuild that year."""
...
def main() -> None:
    ...
    match = re.search(r"american_civil_war_(186[1-5])_editor_backup", args.backup.name)
    if not match:
        raise ValueError(f"Unrecognized backup filename: {args.backup.name}")
    year = match.group(1)
    target = manage.year_dir(year)
    ...
    rows = [row for row in manage.read_csv(manage.CATALOG) if row["year"] == year]
    if not rows:
        raise ValueError(f"No active catalog entries for {year}")
    manage.build_year(year, rows)
    subprocess.run([sys.executable, str(PRACTICE_DIR / "build_index.py")], check=True)
    print(f"Imported {args.backup} and rebuilt {year} with {len(rows)} battle entries")
