#!/usr/bin/env python3
"""Import the newest AI-course Shiji editor backup."""
...
if __name__ == "__main__":
    ...
    if supplied is None:
        candidates = list(
            (Path.home() / "Downloads").glob("ai_course_editor_backup*.json")
        )
        if not candidates:
            raise SystemExit("No ai_course_editor_backup*.json found in Downloads.")
        supplied = max(candidates, key=lambda path: path.stat().st_mtime)
    sys.argv = [
        sys.argv[0],
        str(supplied),
        "--clean",
        str(BASE_DIR / "ai_course_clean.txt"),
        "--dictionary",
        ...
        "--reading-notes",
        str(BASE_DIR / "ai_course_reading_notes.txt"),
        "--backup",
        str(BASE_DIR / "ai_course_clean_before_editor.txt"),
    ]
    shared.main()
