#!/usr/bin/env python3
"""Import the newest 《劳山道士》 editor backup and persist its media."""
...
def write_seed(export_path: Path) -> int:
    ...
    (BASE_DIR / "laoshan_daoshi_editor_seed.json").write_text(
...
if __name__ == "__main__":
    ...
    if supplied is None:
        candidates = list((Path.home() / "Downloads").glob("laoshan_daoshi_editor_backup*.json"))
        if not candidates:
            raise SystemExit("No laoshan_daoshi_editor_backup*.json found in Downloads.")
        supplied = max(candidates, key=lambda path: path.stat().st_mtime)
    sys.argv = [
        sys.argv[0], str(supplied),
        "--clean", str(BASE_DIR / "laoshan_daoshi.txt"),
        "--dictionary", str(BASE_DIR / "laoshan_daoshi_rare_words.csv"),
        "--review-notes", str(BASE_DIR / "review_notes.tsv"),
        "--inline-notes", str(BASE_DIR / "inline_notes.tsv"),
        "--reading-notes", str(BASE_DIR / "laoshan_daoshi_reading_notes.txt"),
        "--backup", str(BASE_DIR / "laoshan_daoshi_clean_before_editor.txt"),
    ]
    shared.main()
