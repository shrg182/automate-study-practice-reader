...
function installBookToggles(){{document.querySelectorAll('[data-book]').forEach(book=>{{book.classList.add('book-collapsed');book.querySelector('.book-toggle')?.setAttribute('aria-expanded','false')}});document.addEventListener('click',event=>{{const toggle=event.target.closest?.('.book-toggle,.contents-toggle');if(!toggle)return;const book=toggle.closest('[data-book]');if(!book)return;if(toggle.classList.contains('book-toggle')){{const collapsed=book.classList.toggle('book-collapsed');toggle.setAttribute('aria-expanded',String(!collapsed));if(!collapsed)book.scrollIntoView({{behavior:'smooth',block:'nearest'}})}}else{{const collapsed=book.classList.toggle('contents-collapsed');toggle.setAttribute('aria-expanded',String(!collapsed))}}}})}}
