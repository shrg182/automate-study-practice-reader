...
function installCategoryControls(){{document.querySelectorAll('[data-category-filter]').forEach(button=>button.addEventListener('click',()=>applyCategory(button.dataset.categoryFilter)));document.querySelectorAll('[data-category-toggle]').forEach(button=>{{const group=button.closest('[data-category-group]');group.classList.add('collapsed');button.setAttribute('aria-expanded','false');button.addEventListener('click',()=>{{const collapsed=group.classList.toggle('collapsed');button.setAttribute('aria-expanded',String(!collapsed))}})}});applyCategory(currentCategory,false)}}
