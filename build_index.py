...
def marxist_books_html(entries: list[dict[str, str | None]], start_number: int) -> str:
    ...
    for group in marxist_book_groups(entries):
        ...
        books.append(f'''<section class="book-group book-collapsed" data-book data-search="{escape(book_search, quote=True)}">
    <header class="book-header">
      <button class="book-toggle" type="button" aria-expanded="false" aria-controls="{group['key']}-contents"><span><span class="book-level">01 · 书名</span><strong>{escape(str(group['title']))}</strong><small>{escape(str(group['meta']))}</small></span><i aria-hidden="true">▾</i></button>{tool_line}
    </header>
...
function installCollectionToggles(){{document.querySelectorAll('[data-collection]').forEach(section=>{{setCollectionCollapsed(section,true,false);section.querySelector('.collection-toggle')?.addEventListener('click',()=>setCollectionCollapsed(section,!section.classList.contains('collapsed'),false))}});document.querySelectorAll('.masthead-books a[href^="#"]').forEach(link=>link.addEventListener('click',event=>{{event.preventDefault();const section=document.querySelector(link.getAttribute('href')),opening=section?.classList.contains('collapsed');document.querySelectorAll('[data-collection]').forEach(item=>setCollectionCollapsed(item,true,false));if(opening&&section){{setCollectionCollapsed(section,false,false);history.replaceState(null,'',link.getAttribute('href'));section.scrollIntoView({{behavior:'smooth',block:'start'}})}}}}));const linked=location.hash&&document.querySelector(location.hash);if(linked?.matches('[data-collection]'))setCollectionCollapsed(linked,false,false)}}
function installBookToggles(){{document.querySelectorAll('[data-book]').forEach(book=>{{book.classList.add('book-collapsed');book.querySelector('.book-toggle')?.setAttribute('aria-expanded','false');book.querySelector('.book-toggle')?.addEventListener('click',()=>{{const collapsed=book.classList.toggle('book-collapsed');book.querySelector('.book-toggle').setAttribute('aria-expanded',String(!collapsed))}});book.querySelector('.contents-toggle')?.addEventListener('click',()=>{{const collapsed=book.classList.toggle('contents-collapsed');book.querySelector('.contents-toggle').setAttribute('aria-expanded',String(!collapsed))}})}})}}
