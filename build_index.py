...
COLLECTIONS = {
    ...
    "nine_commentaries": Collection("nine_commentaries", "《九评共产党》", "章节阅读与校读材料", "专题阅读"),
...
def entry_card(entry: dict[str, str | None], number: int) -> str:
    ...
    return f'''<article class="entry" data-search="{escape(str(entry['search']).casefold(), quote=True)}" data-editor-path="{escape(str(entry['editor']), quote=True)}">
      ...
      <div class="entry-copy"><span>{escape(str(entry['context']))}</span><h3><a class="entry-title-link" href="{escape(str(entry['editor']), quote=True)}?view=annotated" target="_blank" rel="noopener">{escape(str(entry['title']))}</a></h3></div>
      <div class="entry-reading"><strong>未读</strong><time></time></div>
      <div class="entry-editing"><strong>未编辑</strong><time></time></div>
      <div class="entry-action entry-editor"><a class="primary" href="{escape(str(entry['editor']), quote=True)}?view=annotated" target="_blank" rel="noopener">打开编辑器</a></div>
...
def build_html(grouped: dict[str, list[dict[str, str | None]]]) -> str:
    ...
    active = [(key, entries) for key, entries in grouped.items() if entries or key in {"guwen_guanzhi", "laozi", "sunzi", "thirty_six_stratagems"}]
    nav = "".join(
    ...
    for key, entries in active:
        ...
        selector_links = {"rongzhai_suibi": "rongzhai_suibi/select_articles.html", "guwen_guanzhi": "guwen_guanzhi/select_articles.html", "laozi": "laozi/select_chapters.html", "sunzi": "sunzi/select_entries.html", "thirty_six_stratagems": "thirty_six_stratagems/select_entries.html"}
  ...
  <header class="collection-header">
    <div><p>{escape(collection.eyebrow)}</p><h2>{escape(collection.title)}</h2><span>{escape(collection.description)}</span></div>
    <div class="collection-meta">{selector_link}<strong>{len(entries)} 篇</strong></div>
  </header>
  <div class="entries">{"".join(cards)}</div>
</section>''')
...
.collection{{margin-top:34px;scroll-margin-top:90px}}.collection-header{{display:flex;justify-content:space-between;gap:25px;align-items:end;padding:0 3px 15px;border-bottom:2px solid var(--ink)}}.collection-header p{{margin:0 0 6px;color:var(--red);font-size:11px;font-weight:800;letter-spacing:.16em}}.collection-header h2{{margin:0;font:700 28px/1.25 "Songti SC","STSong",serif}}.collection-header span{{display:block;margin-top:7px;color:var(--muted);font-size:13px}}.collection-header strong{{white-space:nowrap;color:var(--muted);font-size:13px}}
.collection-meta{{display:flex;gap:10px;align-items:center}}.collection-tool{{padding:7px 10px;border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--red);text-decoration:none;font-size:12px;font-weight:700}}
...
.masthead-inner{{width:100%;padding:14px 24px;grid-template-columns:minmax(0,1fr) auto;gap:24px;align-items:center}}
...
.controls{{top:0;grid-template-columns:minmax(260px,520px) minmax(0,1fr);gap:10px;margin:0;padding:8px 6px;background:#fff;border-bottom:1px solid var(--line);backdrop-filter:none}}
...
.collection-header p{{display:none}}.collection-header h2{{font:600 16px/1.3 Arial,"PingFang SC",sans-serif}}.collection-header span{{margin-top:2px;font-size:11px}}
...
html[data-workspace-skin="reading"] .collection{{margin-top:34px;scroll-margin-top:90px}}html[data-workspace-skin="reading"] .collection-header{{padding:0 3px 15px;align-items:end;border:0;border-bottom:2px solid var(--ink);background:transparent}}html[data-workspace-skin="reading"] .collection-header p{{display:block}}html[data-workspace-skin="reading"] .collection-header h2{{font:700 28px/1.25 "Songti SC","STSong",serif}}html[data-workspace-skin="reading"] .collection-header span{{margin-top:7px;font-size:13px}}html[data-workspace-skin="reading"] .collection-tool{{border-radius:6px;background:var(--panel);color:var(--red)}}
...
<body>
<header class="masthead"><div class="masthead-inner"><div><p class="kicker">Reading workspace</p><h1>校读书房</h1><p class="lede">文章、古籍与课程材料的阅读编辑器目录。搜索篇名，或按系列进入清稿、注音、脚注、按语与札记工作区。</p></div><div class="count"><b>{total}</b><span>个阅读编辑器</span></div></div></header>
<main class="shell">
  <div class="controls"><label class="search"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="7"/><path d="m20 20-4-4"/></svg><input id="catalogSearch" type="search" placeholder="搜索文章、篇章或系列…" autocomplete="off"></label><nav class="collection-nav">{nav}</nav></div>
  {"".join(sections)}
  <p class="empty" id="emptyState">没有找到匹配的阅读材料。</p>
</main>
<footer><strong>教育用途说明：</strong>本站仅供个人阅读、校读与学习，不隶属于所引用的原文来源网站；原始资料的权利归其相应权利人所有。<br>目录由 <code>practice/build_index.py</code> 自动生成 · 新增编辑器后重新运行即可更新</footer>
<script>
...
window.addEventListener('focus',renderHistory);window.addEventListener('storage',renderHistory);renderHistory();
</script>
</body>
</html>'''
