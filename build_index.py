...
function installCollectionToggles(){{let state={{}};try{{state=JSON.parse(localStorage.getItem('reading-workspace-collapsed-collections')||'{{}}')}}catch{{}}document.querySelectorAll('[data-collection]').forEach(section=>{{setCollectionCollapsed(section,Boolean(state[section.id]),false);section.querySelector('.collection-toggle')?.addEventListener('click',()=>setCollectionCollapsed(section,!section.classList.contains('collapsed')))}})}}
