...
{resources_block}
  <div class="entries{' hierarchical-entries' if key == 'marxist_classics' else ''}" id="{key}-entries">{cards_html}</div>
    ...
    for category, (title, description) in CATEGORIES.items():
        ...
        if not collections:
            continue
        category_groups.append(f'''<section class="category-group collapsed" data-category-group="{category}">
  <header class="category-header"><button type="button" data-category-toggle aria-expanded="false"><span><strong>{escape(title)}</strong><small>{escape(description)}</small></span><b>{category_counts[category]} entries</b><i aria-hidden="true">▾</i></button></header>
...
.collection-description,.collection-meta,.collection-resources{{display:none!important}}.collection-header{{border-bottom:1px solid var(--line)!important}}
...
@media(max-width:760px){{.masthead-inner{{grid-template-columns:1fr;padding-top:45px}}.count{{display:none}}.controls{{grid-template-columns:1fr}}.entry{{grid-template-columns:38px 1fr}}.entry-actions{{grid-column:2;justify-content:flex-start}}.collection-header{{align-items:center;gap:8px}}.collection-toggle-copy,.collection-title{{display:block!important}}.collection-description,.collection-eyebrow{{display:none!important}}}}
